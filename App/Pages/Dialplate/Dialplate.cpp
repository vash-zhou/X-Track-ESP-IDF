...
Dialplate::Dialplate() : recState(RECORD_STATE_READY), lastFocus(nullptr)
{
}
...
{
    // 测试用
    Model.sportStatusInfo.singleTime = 0;
    Model.sportStatusInfo.singleDistance = 0;
    Model.sportStatusInfo.singleCalorie = 0;
    Model.sportStatusInfo.speedKph = 0;
    Model.sportStatusInfo.speedAvgKph = 0;
}
...
void Dialplate::Update()
{
    ...
    snprintf(buf, sizeof(buf), "%.2d", (uint16_t)Model.GetSpeed());
    lv_label_set_text_fmt(View.ui.topInfo.labelSpeed, "%s", buf);

    memset(buf, '\0', sizeof(buf));
    snprintf(buf, sizeof(buf), "%.1f km/h", Model.GetAvgSpeed());
    lv_label_set_text_fmt(View.ui.bottomInfo.labelInfoGrp[0].lableValue, "%s", buf);
    ...
    memset(buf, '\0', sizeof(buf));
    snprintf(buf, sizeof(buf), "%.1f km", Model.sportStatusInfo.singleDistance / 1000);
    lv_label_set_text_fmt(View.ui.bottomInfo.labelInfoGrp[2].lableValue, "%s", buf);

    lv_label_set_text_fmt(View.ui.bottomInfo.labelInfoGrp[3].lableValue, "%d k", uint16_t(Model.sportStatusInfo.singleCalorie));
}
...
void Dialplate::onRecord(bool longPress)
{
    ...
    case RECORD_STATE_READY:
        if (longPress)
        {
            ...
            {
                DC_LOG_WARN("GPS has not ready, can't start record");
